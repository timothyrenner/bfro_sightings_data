...
def main(report_locations_file, report_file, report_join_file, precision):
    ...
    valid_latitudes = (report_locations.latitude >= -90.0) & (
        report_locations.latitude <= 90.0
    )
    valid_longitudes = (report_locations.longitude >= -180.0) & (
    ...
    report_locations.loc[:, "date"] = pd.to_datetime(
        report_locations.timestamp
    ).dt.to_period("d")
    ...
    reports.loc[:, "number"] = reports.loc[:, "number"].astype(int)
    ...
    reports_joined.loc[:, "geohash"] = np.where(
        reports_joined.latitude.isnull(), np.NaN, reports_joined.geohash
    )
